...
`js/intellisense.js` reads it over jsDelivr:

```
https://cdn.jsdelivr.net/gh/Tribhuvanachar/bhumandala@wordnet-dist/_wordnet
```

which is what `appConfig.wordnetDataBase` in `js/config.js` is set to. The
...
SLP1 characters of the word, the same rule `data/_morph` and
`data/_synonyms` use, so one lookup rule serves all three. `manifest.json`
...
then commit `data/_wordnet/**` here as `_wordnet/**`. `.github/workflows/
