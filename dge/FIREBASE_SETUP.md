...
| Cloud Functions (WhatsApp OTP + broadcasts) | ⏳ Fourth attempt (run 4, 8 Sep ~7:09 pm IST), after the lead enabled Cloud Build API, got past all three API-enable checks (Artifact Registry, Cloud Functions, Cloud Build all confirmed enabled) and hit a **new, different blocker**: `Missing permissions required for functions deploy. You must have permission iam.serviceAccounts.ActAs on service account *@appspot.gserviceaccount.com.` A functions deploy runs the function AS the project's default App Engine service account, so the deploying account (`firebase-adminsdk-fbsvc@sarvamula-org.iam.gserviceaccount.com`) additionally needs the **"Service Account User"** role (`roles/iam.serviceAccountUser`) — this is IAM, not an API-enable, so it needs a role grant rather than a console "Enable" click. The lead needs to open https://console.cloud.google.com/iam-admin/iam?project=sarvamula-org, find `firebase-adminsdk-fbsvc@sarvamula-org.iam.gserviceaccount.com`, and add the "Service Account User" role, then re-dispatch `Deploy — Firebase Functions` |
