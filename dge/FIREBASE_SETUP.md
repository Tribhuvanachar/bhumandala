...
| Cloud Functions (WhatsApp OTP + broadcasts) | ⏳ Fifth attempt (run 5, 8 Sep ~7:33 pm IST), after the lead granted "Service Account User" on `firebase-adminsdk-fbsvc`, got the furthest yet — past every API-enable check AND the IAM ActAs check, into actually loading the function source and starting the deploy — then hit a **fourth, different blocker**: `Request to secretmanager.googleapis.com ... HTTP Error: 403, Secret Manager API has not been used in project *** before or it is disabled.` The functions declare secret bindings (`WHATSAPP_TOKEN` etc., see `functions/index.js`) and Firebase checks Secret Manager for them at deploy time even though `push-firebase-function-secrets.yml` hasn't been run yet — so this blocks before secrets even matter. Same one-API-at-a-time pattern as Cloud Build/Artifact Registry/Cloud Functions above. The lead needs to open https://console.cloud.google.com/apis/library/secretmanager.googleapis.com?project=sarvamula-org and click Enable, then re-dispatch `Deploy — Firebase Functions`. **Once this succeeds**, the next step is `push-firebase-function-secrets.yml` to actually populate the WhatsApp/MSG91 secret values (§0.2 step 5) — Enable here only makes the API reachable, it does not create the secrets themselves |
