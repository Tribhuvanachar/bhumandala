...
- **A dedicated Madhva AI Assistant ("Madhva-Vani") — Hybrid RAG (BM25 + vector search) over a commentary-hierarchy knowledge graph, plus a fine-tuned LLM for Dvaita-style Vākyārtha reasoning, across all 37 Sarvamūla Granthas, their Ṭīkās/Ṭippaṇīs, and Dāsa Sāhitya. Proposed 18 Aug 2026; not started. Recorded here rather than acted on because it's a multi-month system, not a same-day feature — but checked against what already exists first, so this is a real gap analysis, not a blind copy of the proposal.**

  **The proposal's premise is right and matches a decision this project already made once, independently.** Pure fine-tuning blends commentator views and can't cite verbatim — the exact failure mode the existing "Intelligence mode" idea above already worried about for cross-references, and the reason Ask Acharya (`dge/js/ai.js`, F-014) is a per-selection live Gemini call grounded in the one shloka card the user actually selected, not an ungrounded chat. The proposal's architecture is the natural next step of that same instinct, scaled to the whole corpus instead of one card at a time.

  **More of the proposal's "Step 1: Structured Corpus Preparation" already exists than the proposal assumes — worth knowing before anyone starts building it as if from zero:**
  - Its `commentary_tree` (Mūla → Ṭīkā → Ṭippaṇī) isn't a design to build, it's already the on-disk shape: `dge/data/dvaitavedanta/dasha_prakarana_granthas/*/{mula, tika_jayatirtha, tippani}`, and the wider Sarvamūla prasthānas (`gita_prasthana`, `sutra_prasthana`, `upanishad_prasthana`, `itihasa_prasthana`, `purana_prasthana`, `sruti_prasthana`) plus `later_acharyas` (Vyāsatīrtha's Nyāyāmṛta/Tarkatāṇḍava/Tātparyacandrikā, Rāghavendra Tīrtha's Parimala under `dasa_sahitya/vyasakuta/raghavendra_tirtha/parimala`, Vādirāja's Yuktimallikā, etc.) are already real, populated folders, not a schema to invent.
  - Its Haridāsa Sāhitya JSON shape (`ankita_nama`, `raga_tala`, category) isn't a proposal either — it's already `schemas.json`'s `dasa_sahitya_composition` and `dasa_pada_text` schemas verbatim, including `ankita_nama` ("the composer's signature/pen-name embedded in the composition"), `raga_tala`, `form` (pada/suladi/ugābhoga/dandaka/mundige/...), `deity`, and axis:value `tags`. `dge/data/dasa_sahitya/{dasakuta,vyasakuta}/<person>/<form>/` already exists per-Dāsa and per-form; most leaves are still empty placeholders (`tools/dasa_sahitya/` is the importer for them), so the *content* is unfinished, not the *schema*.
  - What genuinely doesn't exist yet: a uniform `Adhyāya → Pāda → Adhikaraṇa → Sūtra` addressable ID scheme across every Sarvamūla work the way the Aṣṭādhyāyī already has one (`"1.2.27"` style, noted in the Intelligence-mode idea above) — Brahmasūtra-Bhāṣya-derived works don't carry that consistently today. That's real, unstarted prep work this proposal is right to flag.

  **What's genuinely new and unbuilt, matching the proposal's own claim:**
  - No vector store (Qdrant/pgvector/Milvus) or dedicated BM25 keyword index over Sanskrit/Kannada technical terms anywhere in the repo. `dge/search_index` (916 granthas / 94,664 units, on the `search-dist` branch per the search-reindex item elsewhere in this file) is a corpus-wide text search index, not a semantic/hybrid retrieval layer, and has never been evaluated for this use.
  - No query router or intent classifier distinguishing a philosophical (Sarvamūla/Ṭīkā) query from a Dāsa Sāhitya query.
  - No citation assembler building a full Adhikaraṇa + commentary-tree context window before calling an LLM — Ask Acharya's context today is one selected passage, not an assembled multi-layer citation tree.
  - No fine-tuning dataset (the proposal suggests 1,000–5,000 Q&A pairs in explicit Viṣaya/Saṃśaya/Pūrvapakṣa/Siddhānta form) and no fine-tuning pipeline. This project's existing AI usage is entirely BYOK live-call (Gemini key in `localStorage`, same pattern noted for the batch-padaccheda idea above) — never a hosted/fine-tuned model, which is a materially different cost and infrastructure commitment (Vertex AI or equivalent hosting) than anything else this project runs today.
  - No knowledge-graph layer explicitly linking a Dāsa Sāhitya composition to the Sarvamūla tattva it expresses — `parampara.json`'s free-text `works` arrays (noted in the Grantha-acquisition item below, "Saint → his vyākhyānas does not exist yet") is the closest existing piece, and it's exactly the same joining-problem this proposal's knowledge graph would also need solved.

  **Cost/feasibility follow-up, asked and answered 18 Aug 2026 — not built, this is the analysis for whoever picks it up:**

  - **What it actually needs, in order:** (1) the sūtra-addressing scheme flagged above, since nothing else can cite precisely without it; (2) a retrieval layer — embeddings for semantic match, plus keyword/exact-match for technical terms and śloka numbers; (3) a context/citation assembler; (4) an LLM call, prompted (not fine-tuned, see below) to keep Pūrvapakṣa separate from Siddhānta and cite its sources. Fine-tuning and a knowledge-graph service are the proposal's Phase-2 items, not requirements to ship a first version.

  - **Can we do it in the current situation? Mostly yes, for retrieval — no, unmodified, for hosting.** Every AI feature this project runs today (Ask Acharya, the batch-padaccheda idea, this proposal's own suggested LLM step) is a **live BYOK browser call** against GitHub Pages + jsDelivr static data — there is no server, no database, no always-on process anywhere in this repo. A Qdrant/Milvus/pgvector instance or a Meilisearch/Elasticsearch service needs exactly that: something running continuously, which this project has never had and isn't set up to operate or pay for. **The avoidable part:** this project already ships large read-only datasets as static files on dist branches over jsDelivr — `wordnetDataBase` (24 MB), `koshaDataBase`, `kavyaDataBase` (50 MB) in `dge/js/config.js`. Precomputed, quantized embeddings for the corpus are the same shape of artifact — a static file, not a running service — and a flat/ANN similarity search over them can run **in the browser**, no server, using the same CDN pattern already proven here three times. Sparse/keyword matching has a real head start too: `dge/search_index` already has 916 granthas / 94,664 units indexed (currently plain text search, not scored BM25) and is a nearer starting point than standing up Meilisearch from nothing. So "hybrid retrieval, hosted the way this project already hosts things" is genuinely reachable; "hybrid retrieval via a always-on vector database service" is not, without taking on infrastructure this project has specifically avoided so far (see the GitHub Pages 1 GB / CDN-branch items above — the project has repeatedly chosen static-and-free over anything that costs monthly).

  - **Cost implications, concretely, at current published rates:** Claude Opus 5 is $5/$25 per MTok (input/output), Claude Sonnet 5 $3/$15 ($2/$10 intro through 2026-08-31), Claude Haiku 4.5 $1/$5 — a single well-grounded query (cached system prompt + an assembled Adhikaraṇa/commentary context of a few thousand tokens + a citation-disciplined answer) is on the order of a few cents on Sonnet-tier, less with prompt caching on the repeated system-prompt/citation-format prefix. **Who pays matters more than the per-query price**, though: staying BYOK (today's pattern — the user's own key, same as Ask Acharya) keeps this at $0 to the project regardless of volume, same as every AI feature here today. Switching to a project-funded "free for every visitor" assistant turns that into a real, unbounded-by-usage recurring bill for the first time in this project's life. Embedding generation for the whole corpus is one-time and cheap (well under $50 total at typical embedding-API rates for a few hundred thousand chunks) and only needs re-running when the corpus grows.

  - **Fine-tuning specifically: avoidable, and the recommendation is to avoid it.** Anthropic does not offer self-serve fine-tuning of Claude models at all — that route would mean either Vertex AI tuning of Gemini or self-hosting an open-weight model (Llama/Gemma), and either way adds a **second** always-on, real-money hosting bill on top of retrieval (a dedicated tuned-model serving endpoint typically costs real money per hour whether or not anyone queries it — plausibly hundreds of dollars a month at the low end, against a project whose entire hosting bill today is zero). More importantly it's not needed for what the proposal actually wants: the Pūrvapakṣa/Siddhānta discipline and per-claim citation the system prompt in the original proposal already specifies can be enforced by that same system prompt plus solid retrieved context, on an off-the-shelf frontier model, the same way Ask Acharya already avoids hallucination today by grounding in the selected passage rather than by being a fine-tuned model. Fine-tuning would mainly buy Halagannada/Sandhi-Samāsa fluency at the margins — worth reconsidering only after a prompted-RAG version is built and its failure modes are actually observed, not before.

  **Not started, deliberately — this needs its own scoped design pass, not a fast follow:** which pieces to build first (the proposal's own suggestion — draft the JSON schema, or a hybrid-search pipeline test on a sample text — is reasonable, but the schema half is mostly already done per above, so the real first move is probably the sūtra-addressing scheme plus a hybrid-search prototype over already-populated `dvaitavedanta` content); and, per the cost analysis above, favor a client-side/CDN-hosted retrieval layer plus a prompted (not fine-tuned) LLM call as the default plan unless the project lead specifically wants a project-funded hosted assistant instead of the BYOK pattern used everywhere else here.
...
- **Taxonomy widened per an uploaded "DGE Shastra Taxonomy" reference doc, additive only — the doc's own contradictions and the one real structural problem it surfaced were NOT auto-resolved (18 Aug 2026).** Asked to apply only the doc's final "Recommended DGE top-level taxonomy" section (tree + its "Important classification decisions" + suggested metadata model), not the long traditional tree above it. Done, safely: five new top-level `taxonomy.json` branches (`jyotisha`, `upaveda`, `artha_rajaniti`, `kamashastra`, `vastu_shilpa`) plus five new Vedānta sub-schools (`vishishtadvaita`, `bhedabheda`, `shuddhadvaita`, `dvaitadvaita`, `acintya_bhedabheda`) under `darshana.vedanta`, all as empty container nodes — nothing existing was renamed or moved, so no published path or CDN-pinned commit breaks. `tools/gen_library_status.py` re-run and its output committed (leaves 1651 → 1682). This also gives a taxonomy home to Ayurveda and Kāmaśāstra sources found and zeroed-in-on the same day (see the acquisition entry directly below) — `upaveda.ayurveda.{brihattrayi,laghutrayi,nighantu}` and `kamashastra.{kamasutra,panchashayaka,smaradipika}` name the actual works already sourced.

  **One judgment call made, not just executed blindly:** the doc's own tree lists Arthaśāstra in two places — nested under `Upaveda & Applied Śāstra` *and* as its own top-level `Artha / Rājanīti` sibling — with nothing in its "Important classification decisions" resolving the conflict (decision #3 argues Ayurveda/Dhanurveda/Gandharvaveda into Upaveda specifically, never Arthaśāstra). Picked the standalone `artha_rajaniti` branch as the one home and left it out of `upaveda`, rather than silently keeping the doc's redundant duplicate node.

  **One placement left genuinely open, not guessed:** where general Nītiśāstra (Cāṇakya Nīti, Kāmandakīya Nītisāra, Pañcatantra, Hitopadeśa — sourced earlier today, see below) belongs. The doc doesn't give Nīti its own branch anywhere, in either the full traditional tree or the recommended one. `artha_rajaniti` is the closest natural fit by subject (statecraft + practical ethics) but that's a guess, not something the doc states — noted in `taxonomy.json`'s own `artha_rajaniti._note` rather than acted on.

  **One real structural problem the doc's tree exposed by contradiction, deliberately NOT touched:** the doc's traditional tree files Dvaita Vedānta *nested inside* Darśana → Vedānta, then separately gives it a full independent structure as if it were its own top-level branch — and checking the live repo against that found a genuine pre-existing duplication, not just a documentation inconsistency. `darshana/vedanta/dvaita/sarvamula/` holds real content — **170 files**, not stubs (`dvadasha_stotra`, `dasha_prakarana_granthas/{vishnu_tattva_vinirnaya,tattvodyota,katha_lakshana,tattva_sankhyana,tattva_viveka,mayavada_khandana}`, each with `mula`/`tika_jayatirtha`/`tippani` layers) — while the top-level `dvaitavedanta` branch, covering overlapping ground under different key names (`gita_prasthana`, `sutra_prasthana`, `sruti_prasthana`, `later_acharyas`, etc.), holds **764 files** and is clearly where this project's actual Dvaita-corpus work has been landing. Two live homes for the same tradition, one much larger than the other — worth knowing about, but merging, redirecting, or deleting either is a real content decision (which files are duplicates vs. genuinely distinct editions, what already links to which path) that needs the project lead's own look, not a guess made here.

  **Also not done, on purpose:** the doc's suggested per-record metadata model (`primary_category` / `secondary_categories` / `tradition` / `sampradaya` / `philosophical_school` / etc., replacing single-folder-only placement so one work like the Bhagavad Gītā can carry multiple classifications) is a real, well-motivated idea — this project's own `schemas.json` today has no such fields — but retrofitting it means touching the shape of every one of the ~1,470 catalog `data.json` files, not a same-day addition. Recorded here as a direction worth taking, not attempted.
