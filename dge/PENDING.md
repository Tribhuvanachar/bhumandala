...
- **Vyākaraṇa/Chandas overhaul (overnight autonomous session, 20–21 Aug), per the project lead's brief ("better than ashtadhyayi.com") + 39 reference screenshots.** All shipped on `claude/ashtadhyayi-page-enhancements-syr044`, each piece browser-verified at 360px/768px/1280px before its commit. Summary of what landed, what was found along the way, and what's deliberately not done:
  ...
  - **Still open from the big brief, deliberately untouched tonight** (each needs its own pass and some need decisions): unified `reader.html` route + auto-pagination (§4.1); shabda instant-modal/no-redirect + "where else" engine (§5.1–5.2); search filter pills (§5.3); automated GitHub-issue correction pipeline (§6 — the mailto template is the shipped first block; see the triage-pipeline design note below); floating-toolbar/draggable-dock overhaul and library_manager modernization (§7); नामधातु/कर्मकर्तरि (above); Vedic chandas.
