...
- **Gold-Standard Commentary Contract (v2.2) — gap analysis and integration plan written, 25 Aug, not yet built.** The project lead shared a formal philological/rendering spec (`Gold-Standard Commentary Contract & Universal Sanskrit Exegetical Framework v2.2`), a matching real-content sample (a Gītā-Vivṛtti adhyāya JSON), and a reference reader page, with the directive: "going forward all content will be presented in gold standard... may need schema overhaul for entire library wherever applicable." Full analysis, gap comparison against DGE's actual current schema/render code, and a proposed additive (non-destructive) integration architecture is written up in `dge/GOLD_STANDARD_ARCHITECTURE.md` — read that file for the complete plan; summarized here:
  ...
  - **The visual layout question** ("would be great if such a layout is also available," re: the reference reader's cream/gold Cinzel-titled design): recommended as a 6th theme option inside DGE's existing Display-sheet theme system (already 5 themes, dark-mode-aware, switchable live) rather than a second, parallel reader page — keeps one reader with richer content instead of forking the presentation layer. Not yet built; needs a quick confirm/reject from the project lead before implementation.
  - **Genuinely open, needs the project lead's call before Part B/C get built**: confirm the "6th theme, not a separate page" direction; after that, `gold-render.js` + schema extension is independently scoped and buildable, with the shared Gītā-Vivṛtti sample as a ready-made real (not synthetic) test case; `validate_gold_standard.py` follows; then pick the first real corpus for an end-to-end pilot before committing to any wider backfill of existing plain-string commentary.
