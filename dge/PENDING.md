...
- **The Sanskrit WordNet is built and wired in, and its 24 MB has nowhere to live — the one decision that stands between the feature and the reader.** `tools/build_wordnet.py` turns IndoWordNet's Sanskrit half into `dge/data/_wordnet/`: 37,734 synsets, 80,009 words, 589 buckets, 23.8 MB. `js/intellisense.js` reads it (an अर्थः section in the word popover: a Sanskrit definition, a Sanskrit usage sentence, the rest of the synset, the class it falls under, and the Kannada WordNet's words for the same synset). The tree is **gitignored, not committed** — the site measured 1,017 MB with it and about 991 MB without, and 1 GB is the GitHub Pages limit this project spent Round 5 getting back under. Three ways out, none of them mine to pick:
  - **Publish it to a data repo and point `window.WORDNET_DATA_BASE` at jsDelivr**, exactly as `koshaDataBase` already does for the 1.65M-headword kośa corpus. The client already prefers that variable when it is set, so this is one line in `js/config.js` plus a push to the data repo. This is the recommendation: it costs the main repo nothing.
  - **Commit a trimmed build.** `--no-examples` is 17.4 MB, `--no-examples --languages ""` is 14.7 MB. Both still land the site over 1 GB from where it is today, so this only works alongside the deferred Step B slimming above.
  - **Leave it unbuilt.** The code is harmless: the fetch 404s, `dgeWordNet` resolves empty, and the popover renders exactly as it does now.
