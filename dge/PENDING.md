...
- **Merge labeled trigram `pkey-substr` hits underneath exact hits when exact yields < limit** — the compound-interior recall fallback (a word buried mid-compound, e.g. `X°kantaya`, lives in X's bucket and is invisible to the word index; only the trigram path reaches it). Today exact-empty falls back to the full fuzzy path, but exact-sparse does not blend. Design is agreed (append non-duplicate trigram hits below, labeled as near matches); needs UI treatment for the label.
