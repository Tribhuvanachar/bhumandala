...
- **ashtadhyayi-com/data investigated for what could fill the Dhatupatha's known gaps (seT/aniT, kṛt/karma, ādivarṇa/antyavarṇa, anubandha) — one gap closed for real, the rest scoped and left as an explicit numbered list, not silently dropped.** Cloned `github.com/ashtadhyayi-com/data` (commit `24109f7`; README: "free to use... provided that appropriate credits are mentioned") and checked its actual contents rather than assuming from the name:
  ...
  5. **Verb-form tables (लुङन्त, यङ्लुङन्त, सन्नन्त, णिजन्त, कर्मणि, कर्तरि — Issue 15's "Dhātu Bodha" ask) — available, deliberately deferred.** `dhatu/dhatuforms_vidyut_*.txt` (shuddha/san/nich/yang/yangluk × kartari/karmani/krut) exist and are exactly this, but the full `dhatu/` directory is 158 MB and individual tables run 6–17 MB each — against a site already at 992 MB of its ~1 GB GitHub Pages ceiling, this cannot go in as monolithic files the way `data.txt` did. Needs either a size-budget pass elsewhere first or a new per-root chunking importer (same shape as `tools/build_prakriya.py`'s per-code JSON files) before it's safe to bring in — not started.
  6. **Śabda-side data (declension tables, meanings — the other half of Issue 15/19's ask) — available, not evaluated for import yet.** `shabda/data2.txt` (9.8 MB, per-word declined forms), `shabda/shabda_meanings.txt` (15 MB, Apte-style glosses), `shabda/shabdaprakriya.txt` (3 MB) exist under the same licence. Sized more reasonably than the verb-form tables but still needs its own scoping pass (which fields, matched against which local schema) before importing — not started this session.
