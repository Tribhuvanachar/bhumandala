...
- **A batch of 20 more issues (24-43), worked through 19 Aug, one at a time with real browser verification before each commit; a few flagged here rather than silently claimed done.**
  ...
  - **Not yet started: 26 (footer/Contact Us/anti-bot), 27 (name+language onboarding, global language threading), 28 (editable Know More subpages, single sponsors file), 29 (top-menu link to Our Story + voice-input script default), 30 (word-popup Shabda/Dhatu/intelligence actions), 31/33/34 (Kosha auto-linking, search keyword highlighting, broader intelligence/interlinking), 32 (global text-box styling), 36 (hide Kosha cleared-status from end users), 40 (What's New auto-logging), 41 (Dhatu filter collapse + hide Vidyut/API wording).** Continuing in the same session; noting the state here in case it doesn't finish in one sitting.
