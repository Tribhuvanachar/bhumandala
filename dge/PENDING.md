...
- **Gold-Standard Commentary Contract (v2.2) — gap analysis and integration plan written, 25 Aug; design mechanism now settled by direct conversation, build not yet started.** The project lead shared a formal philological/rendering spec (`Gold-Standard Commentary Contract & Universal Sanskrit Exegetical Framework v2.2`), a matching real-content sample (a Gītā-Vivṛtti adhyāya JSON), and a reference reader page, with the directive: "going forward all content will be presented in gold standard... may need schema overhaul for entire library wherever applicable." Full analysis lives in `dge/GOLD_STANDARD_ARCHITECTURE.md` — read that file for the complete plan; summarized here:
  - **The gap is real, not cosmetic**: DGE's current commentary is one plain string per commentary-key, rendered with only search-term highlighting; Gold-Standard adds structured `word_mappings`/`padaccheda`/`anvaya`, a strict `commentary_markdown` dialect (pratīka `**"..."**` spans with exact parity to `word_mappings`, block directives for commentator-verses/citations/summary boxes), and a presentation contract (daṇḍa non-breaking binding, pratīka↔word-pill bidirectional linking, mobile typography rules) — none of which exist in DGE's renderer today. `parseMarkdown()` (ai.js), the one markdown parser already in this codebase, is real but built for free-form AI chat replies and has no concept of any of this.
  - **Proposed as additive, not a rewrite**: the schema extension makes each commentary EITHER the current plain string OR a new `{format:"gold_v2_2", commentary_markdown, word_mappings, ...}` object, chosen per-commentary — legacy content keeps rendering exactly as today, forever, with no forced migration of the existing ~tens-of-thousands-of-units corpus. A new, isolated render module (`gold-render.js`, mirroring `footnote-engine.js`'s own additive-module pattern) handles only the new shape; a new `tools/validate_gold_standard.py` enforces the spec's own V1–V7 CI gates at ingestion time, before any Gold-Standard content merges — reusing `search_toolkit_pkg/normalize.py`'s existing sandhi-fold tables for the sandhi-cleanliness check rather than writing new logic.
  - **The layout-vs-theme question, corrected and settled after the project lead pushed back on this document's first draft**: Gold-Standard rendering is a *Layout* (new content structure — word-mapping pills, provenance boxes, pratīka↔pill linking) not a *Theme* (color/font only, same DOM). Calling it "a 6th theme" was wrong and would have bolted new content onto a system built only to repaint existing content. Settled instead as: built **in place inside `index.html`**, `render.js`'s card-building function branching on `format: gold_v2_2` (same mechanism Scholar/App layout already uses), not a second parallel reader page — no forking the audio player/notes/search/immersive-mode plumbing a separate page would otherwise have to rebuild.
  - **Gold-Standard badge + certificate wrapper, per the project lead's direct ask** ("some badge... gold wrapper, like some things which are wrapped on a certificate"), two elements: (1) a small always-present badge next to the commentary title (reusing the existing `.dge-ai-badge` placement convention, gold-toned instead of gray) that IS the switch into the rich view, present only when `format: gold_v2_2`; (2) a certificate-style gold-bordered wrapper around the commentary block, applied only once that rich view is active, built from theme-aware custom properties so it still reads correctly across all 5 existing themes and dark/light. Both key off the same `format` field and both are simply absent for legacy content — no separate flag, no risk of legacy content looking falsely "certified."
  - **Genuinely open — build order only, the design calls above are resolved**: `gold-render.js` + schema extension (with the badge/wrapper built as part of the same unit, not deferred) is independently scoped and buildable, with the shared Gītā-Vivṛtti sample as a ready-made real (not synthetic) test case; `tools/validate_gold_standard.py` follows; then pick the first real corpus for an end-to-end pilot before committing to any wider backfill of existing plain-string commentary.
...
- **`taxonomy.json`: at least 47 `_default_author` fields hold mis-scraped body text, not an author name — found 23 Aug while building `dge/data/commentators.json` (see above).** Concentrated in a handful of ingestion batches — `dvaitavedanta.later_acharyas.karmavijaya.*` and `.nyaya_sudha.*` are the worst of it (whole `tika_*` KEYS there are also garbage, auto-slugified from the first few words of body text, e.g. `tika_bahunam_vacanikarthanam_tatparyarthena_nigamanamiti_kham_cam_mat`), plus scattered singles in `dasha_prakarana_granthas.prapancha_mithyatvanumana_khandana`, `purana_prasthana.bhagavata_tatparya_nirnaya`, `upanishad_prasthana.{chandogyopanishad,taittiriyopanishad}_bhashya`, `later_acharyas.candrikamandanam`, and `sutra_prasthana.anuvyakhyana`. Two of the original 22 flagged instances were recoverable without guessing — the `tika_*iya`/`tika_*iyatippani` KEY itself already encodes the author via a standard Sanskrit possessive suffix (`tika_shrinivasatirthiyatippani` → Śrīnivāsa Tīrtha; `tika_satyadharmiya` → Satyadharma Tīrtha) — both folded into the registry. **The remaining ~45+ are not recoverable from the data as it stands**; whatever ingestion step produced these batches needs to be re-run against a source that actually has the tīkā's heading, or corrected by hand against a real edition. Full list of affected paths was written out during the investigation but not committed anywhere yet — regenerate by walking `taxonomy.json` for `_default_author` values over ~45 chars with no `श्री`/`तीर्थ`/`आचार्य`/`विरचित`-type name marker, same heuristic used to build the registry.
