...
- **Grantha acquisition list dictated 18 Aug 2026 — 14 lines to source and load, plus a two-way Veda↔saint linking requirement that is half-built. Several titles came through a voice transcription garbled; my readings are recorded beside the raw words rather than silently corrected, and the flagged ones need the project lead's own confirmation before anyone goes hunting for a text. Bṛhatī Sahasra has since been confirmed; two remain open.** Nothing here is sourced yet — this is the wanted-list, not a status report.
  ...
  | "Brihati Sahasra" | Bṛhatī Sahasra — **confirmed by the project lead**: not the bṛhatī chandas itself but an aggregate of mantras, a thousand ṛks, known collectively by that name. So it is a collection to hold as a unit and to hang the saints' vyākhyānas off, not a metre to tag verses with | high | `vedas/` — as a named collection over its constituent ṛks, see the note below the table |
  ...
  **What the Bṛhatī Sahasra clarification means for the data, since it is not just a naming point.** If it is an aggregate of roughly a thousand ṛks rather than a work in its own right, then those ṛks are already here — `vedas/rigveda/shakala_shakha/samhita/mandala_09` holds 1,108 mantras, each with a stable id (`9.1.1`, `9.1.2`, …) alongside its pada-pāṭha, ṛṣi, devatā and chandas. So Bṛhatī Sahasra should be a **manifest over those ids, not a second copy of the text**: a small file naming the collection and listing its constituent mantra references, rendered as a readable sequence and used as the target a saint's vyākhyāna attaches to. Copying the verses into their own folder would give the site two Rigvedas that drift apart, and would split every backlink between them. The same reasoning applies to Puruṣa Sūkta and Pavamāna Sūkta as named sūktas — which is convenient, because the addressing a manifest needs (`<grantha-slug>#<unit-id>`) is exactly what `references[].target` already uses.
