...
- **A batch of five separate reports ("why are these failing again and again") — three real bugs found and fixed with live verification, two confirmed to be genuinely large unbuilt features rather than something silently broken.** Investigated each one directly (four parallel research passes) before touching anything, since a couple of these had been reported as fixed before. 19 Aug.
  ...
  5. **NOT STARTED, genuinely large features rather than deferred bugs — flagged with what's actually missing, not just "later."**
     - **Word-click deep-linking to a specific inflected form, highlighted** (उवाच → the Dhātu page's वच् root, Liṭ-lakāra, highlighted; परस्य → the Śabdapāṭha page's पर entry, ṣaṣṭhī-ekavacana cell, highlighted). Confirmed no reverse lookup (surface form → exact root/headword + lakāra/vibhakti + puruṣa/vacana slot) exists anywhere in the repo for either dhātu or śabda data — `?q=` today is pure substring text search over forward-only fields (root/headword → its forms), feeding a filtered list, never an auto-opened-and-highlighted specific entry. Buildable without external sourcing (`shabda`'s reverse index is a straightforward inversion of the existing fixed 24-slot `forms` layout; `dhātu`'s Liṭ/शुद्ध-कर्तरि forms already exist as precomputed data via `tools/build_prakriya.py`'s output) but needs: a real reverse-index generation pass, new deep-link URL params on both dhatu.html/prakriya.html and shabda.html, and highlight-the-specific-cell UI on both — a genuine multi-file feature build, not a quick fix.
