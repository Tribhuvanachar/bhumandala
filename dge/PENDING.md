...
- **Convert tool (v0.26.0–0.27.0): root-caused and fixed the "OCR says choose a file again after backgrounding the tab" report, plus batched Vision OCR calls (real speedup, project lead's own suggestion).**
  ...
  **Not built without the project lead's direction first** (asked, awaiting reply): (a) whether to build a truly unattended background-processing path — most concretely, a GitHub Actions workflow that runs OCR+Proofread server-side so it survives the phone/browser closing entirely — since that's a real architecture and security decision (API keys would need to live as GitHub Secrets, the PDF needs a way into the workflow, GitHub's free-tier job timeout is 6 hours), not a client-side tweak; (b) whether to build automatic OCR→Proofread pipeline overlap (Proofread currently CAN be run manually while OCR is still going — nothing blocks clicking both buttons — but it only proofreads whatever's in the in-memory OCR list at the moment it's clicked, not automatically as new pages keep finishing).
