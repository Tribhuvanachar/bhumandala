...
- **Still needs the project lead**: the workflow needs **Settings → Actions → General → Workflow permissions → "Read and write permissions"** + **"Allow GitHub Actions to create and approve pull requests"** enabled — no tool available to this session can flip that setting; it must be done once, manually, in GitHub's UI, before the "Run workflow" button will actually work end-to-end.
...
- Enable **Settings → Actions → General → Workflow permissions** ("Read and write" + "Allow GitHub Actions to create and approve pull requests") — required before the new ingest workflow's PR step will work; no session tool can set this.
