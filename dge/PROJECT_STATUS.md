...
- Deferred, not attempted this session: Phase 5 (GitHub Actions automation of this pipeline — needs a decision on where automation credentials would live, since all keys are currently BYOK/client-side only) and Phase 6 (Lipi Gnani integration — the source spec itself says not to make it mandatory until its implementation/licence/deployment feasibility is verified; not yet researched).
